...
public class DangNhapServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        session.setAttribute("ThongBao", "Bạn đã đăng xuất");
    ...
    throws ServletException, IOException {
        ...
        HttpSession session = request.getSession();
        if(nd ==null){
            session.setAttribute("ThongBao", "Tài khoản hoặc mật khẩu không đúng!");
            response.sendRedirect("Home");
        }
        else{
            if(nd.isVaiTro()){
                AdminServlet adm = new AdminServlet();
                session.setAttribute("ThongBao", "Đăng nhập với quyền Admin");
                adm.doGet(request, response);
            }
            else{
                session.setAttribute("ThongBao", "Đang nhập với quyền người dùng");
                HomeServlet home = new HomeServlet();
                home.nd = nd;
                home.doGet(request, response);
            }
